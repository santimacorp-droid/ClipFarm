...
const TaskProgress: React.FC<TaskProgressProps> = ({ 
  ...
  useEffect(() => {
    if (status === 'processing') {
      // Immediately get once
      fetchTaskProgress();
      
      // Every5Update per second
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      ...
      case 'completed':
        return { icon: <CheckCircleOutlined />, color: 'success', text: 'Done' };
      case 'failed':
      ...
      <Card size="small" style={{ marginBottom: 16 }}>
        <div style={{ textAlign: 'center', padding: '20px' }}>
          ...
          <div style={{ marginTop: 16 }}>
            <Text>In progress: getting task progress...</Text>
          </div>
        </div>
      </Card>
    );
    ...
    <Card size="small" style={{ marginBottom: 16 }}>
      ...
      <div style={{ fontSize: '12px', color: '#999' }}>
        <Space split={<Text type="secondary">|</Text>}>
          ...
          {progressData.started_at && (
            <Text type="secondary">
              Starting: {new Date(progressData.started_at).toLocaleString()}
            </Text>
          )}
          {progressData.completed_at && (
            <Text type="secondary">
              Finished: {new Date(progressData.completed_at).toLocaleString()}
            </Text>
          )}
        </Space>
      </div>
      ...
      {status === 'processing' && (
        <div style={{ marginTop: 16, textAlign: 'center' }}>
          <Text type="secondary">Progress every5Automatic update per second</Text>
        </div>
      )}
    </Card>
  );
