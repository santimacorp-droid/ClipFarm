...
export interface Clip {
  id: string
  title?: string  // Original may not existtitle
  start_time: string
  end_time: string
  final_score: number  // Match backend field name
  recommend_reason: string  // Match backend field name
  generated_title?: string
  outline: string
  content: string[]
  chunk_index?: number  // Add missing fields
  video_url?: string
  video_path?: string
  clip_metadata?: any
...
// Project status type definitions, kept in sync with backend
...
export const useProjectStore = create<ProjectStore>((set, get) => ({
  ...
  setProjects: (projects) => {
    ...
    // Skip update if user is dragging to avoid conflicts
    if (state.isDragging) {
  ...
  reorderCollectionClips: async (projectId: string, collectionId: string, newClipIds: string[]) => {
    ...
    // Prioritize items fromcurrentProjectScan here; scan again inprojectsSearch within array
    ...
    // IfcurrentProjectIf not found here, try fromprojectsSearch within array
